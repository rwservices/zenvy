...
if ( $property_types ) :
	...
	$post_limits = get_theme_mod( 'zenvy_front_page_property_type_limits', [ 'desktop' => 6 ] );
	?>
			...
			<div class="custom-tabs">
				<ul class="tab-links">
					<?php foreach ( $property_types as $property_key => $property_type ) : ?>
						<?php
						if ( ! empty( $property_type['cat_slug'] ) ) :
							$property_term_type = get_term_by( 'slug', $property_type['cat_slug'], 'property-type' );
							?>
				...
				<div class="tab-content">
					<?php foreach ( $property_types as $property_key => $property_type ) : ?>
						...
						if ( ! empty( $property_type['cat_slug'] ) ) :
							...
							?>
							<?php if ( $property_term_type ) : ?>
								<div id="<?php echo esc_attr( $property_term_type->slug ); ?>" class="tab
								<?php
								if ( $property_key === 0 ) {
									?>
									active<?php } ?>">
									...
									<?php endif; ?>
									<?php
									$enable_view_all = get_theme_mod( 'zenvy_front_page_property_type_view_all_btn', '' );
									if ( $enable_view_all && array_key_exists( 'desktop', $enable_view_all ) ) :
										?>
										<div class="d-flex justify-content-center btn-wrap">
											<span class="box-button view-all-btn"><a href="<?php echo esc_url( get_category_link( $property_term_type->term_id ) ); ?>"><?php esc_html_e( 'View All', 'zenvy' ); ?></a></span>
										</div>
										<?php
									endif;
									?>
