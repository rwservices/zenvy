...
( function( $, api ) {
	 ...
	 */
	const zenvy_linked = ( id, key ) => {
		const holder = $( '.customize-control .control-wrap.' + key );
	 ...
	 * @param customizerTab
	 */
	const zenvy_hide_controls = ( customizerSection, customizerTab ) => {
		const newControls = [];

		$( customizerSection ).find( '#' + customizerTab + ' .customizer-tab' ).each( function() {
			const input_wrap = $( this ).find( 'input' ),
	 ...
	 */
	const zenvy_show_controls = ( customizerSection, controlsToShowArray ) => {
		$.each( controlsToShowArray, function( index, controlId ) {
			const parentSection = customizerSection[ 0 ];
	...
	$( document ).ready( function() {
		...
		$( '.control-wrap .link-dimensions .unlinked' ).on( 'click', function() {
			const holder 	= $( this ).closest( '.control-wrap' ),
		...
		$( '.control-wrap .dimensions' ).on( 'input', '.dimension-input', function() {
			const dataElement = $( this ).attr( 'data-element' ),
		...
		$( '.control-wrap' ).on( 'input change', 'input[type="range"].range', function() {
			const input_number = $( this ).closest( '.control-wrap' ).find( '.range-input input[type="number"]' ),
				value = $( this ).val();
		...
		$( '.custom-toggle-btn-wrap input[type="checkbox"]' ).on( 'click', function() {
			const toggle = $( this ).closest( '.control-wrap.active' ).find( '.custom-toggle-btn-wrap .custom-toggle-btn' ),
		...
		$( '.fa-icons-list li' ).on( 'click', function() {
			...
			const iconVal = $( this ).parents( '.icons-list-wrapper' ).find( 'li.selected' ).children( 'i' ).attr( 'class' );
			const inpiconVal = iconVal.split( ' ' );
	...
	$( document ).ready( function() {
		...
		$( '.customizer-group-control' ).each( function() {
			const customizerSection = $( this ).closest( '.accordion-section' ),
				customizerTab = $( this ).closest( '.customize-control-zenvy_group' ).attr( 'id' ),
				shownCtrls = $( this ).find( '.customizer-tab > .active' ).data( 'controls' );

			// Hide all controls in section.
			zenvy_hide_controls( customizerSection, customizerTab );

			// Show controls under first radio button.
			zenvy_show_controls( customizerSection, shownCtrls.split( ',' ) );
		} );
	} );

	api.sectionConstructor.zenvy_custom_section = api.Section.extend( {
	...
	/**
	 * Customizer Control: zenvy_background
	 */
	api.controlConstructor.zenvy_background = api.Control.extend( {
		...
		ready() {
			...
			control.container.on( 'click', '.background-image .attachment-media-view .upload-button', function( e ) {
				...
				const image = wp.media( { multiple: false } ).open().on( 'select', function() {
			...
			Object.keys( fields ).forEach( function( item ) {
				...
				if ( item === 'gradient' ) {
					[ 'color_1', 'color_2' ].forEach( ( key ) => {
						const v = values[ item ] !== undefined ? values[ item ][ key ] : '';
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_background', control.container );
				...
				Object.keys( control.params.fields ).forEach( function( item ) {
					...
					// for colors
						...
						holder.find( '.background-colors .color-picker' ).each( function() {
							const colors = $( this ).find( '.wp-picker-container .alpha-color-control' ),
								r = colors.data( 'reset' );
	...
	/**
	 * Customizer Control: zenvy_border
	 */
	api.controlConstructor.zenvy_border = api.Control.extend( {
				...
				if ( item === 'width' ) {
					...
					if ( values[ item ] !== undefined && values[ item ].linked !== undefined ) {
						if ( values[ item ].linked === 'on' ) {
							zenvy_linked( control.id, 'border-control' );
						} else {
							zenvy_unlinked( 'border-control' );
						}
					}
				}
				// for radius
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_border', control.container );
	...
	/**
	 * Customizer Control: zenvy_box_shadow
	 */
	api.controlConstructor.zenvy_box_shadow = api.Control.extend( {
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_box_shadow', control.container );
	...
	/**
	 * Customizer Control: zenvy_typography
	 */
	api.controlConstructor.zenvy_typography = api.Control.extend( {

		ready() {
			const control = this;

			// Font Options
			control.selectFontOptions( window.zenvyAllFonts.allFonts );
				...
				if ( item === 'font_family' ) {
					...
					if ( v ) {
						...
						if ( typeof id !== 'undefined' ) {
							if ( typeof window.zenvyAllFonts.allFonts[ id ] !== 'undefined' ) {
								const font = window.zenvyAllFonts.allFonts[ id ];
								css[ item ] = font.name;
							}
						}
					}
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_typography', control.container );
		...
		selectFontOptions( fonts ) {
			...
			if ( typeof window.fontFamiliesOptions === 'undefined' ) {
				...
				_.each( fonts, function( font, id ) {
					const html = '<option value="' + id + '">' + font.name + '</option>';
		...
		setFontVariants( font_id ) {
			...
			if ( typeof window.zenvyAllFonts.allFonts[ font_id ] !== 'undefined' && font_id !== '' ) {
				_.each( window.zenvyAllFonts.allFonts[ font_id ].font_variants, function( value, id ) {
		...
		getFontId( fontName ) {
			if ( fontName !== undefined ) {
				const font_id = fontName.toLowerCase();
	...
	/**
	 * Customizer Control: zenvy_sortable
	 */
	api.controlConstructor.zenvy_sortable = api.Control.extend( {
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder 	= $( this ).closest( '.customize-control-zenvy_sortable', control.container );
	...
	/**
	 * Customizer Control: zenvy_group
	 */
	api.controlConstructor.zenvy_group = api.Control.extend( {
		...
		ready() {
			...
			control.container.on( 'click', '.customizer-tab > label', function( e ) {
				...
				// Hide all controls in section
				zenvy_hide_controls( customizerSection, customizerTab );
				zenvy_show_controls( customizerSection, controls.split( ',' ) );
	...
	/**
	 * Customizer Control: zenvy_toggle
	 */
	api.controlConstructor.zenvy_toggle = api.Control.extend( {
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_toggle', control.container );
	...
	/**
	 * Customizer Control: zenvy_color
	 */
	api.controlConstructor.zenvy_color = api.Control.extend( {
		...
		setValues() {
			...
			// for colors
			Object.keys( colors ).forEach( ( key ) => {
				const v = values[ key ] !== undefined ? values[ key ] : '';
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_color', control.container );
	...
	/**
	 * Customizer Control: zenvy_buttonset
	 */
	api.controlConstructor.zenvy_buttonset = api.Control.extend( {
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_buttonset', control.container );
	...
	/**
	 * Customizer Control: zenvy_range
	 */
	api.controlConstructor.zenvy_range = api.Control.extend( {
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_range', control.container );
	...
	/**
	 * Customizer Control: zenvy_dimensions
	 */
	api.controlConstructor.zenvy_dimensions = api.Control.extend( {
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_dimensions', control.container );
				...
				holder.find( '.dimensions-control .link-dimensions input[type="hidden"]' ).each( function() {
					...
					if ( r === 'on' ) {
						zenvy_linked( control.id, 'dimensions-control' );
					} else {
						zenvy_unlinked( 'dimensions-control' );
					}
				} );
	...
	/**
	 * Customizer Control: zenvy_radio_image
	 */
	api.controlConstructor.zenvy_radio_image = api.Control.extend( {
		...
		resetValues() {
			...
			control.container.on( 'click', '.reset-value', function() {
				const holder = $( this ).closest( '.customize-control-zenvy_radio_image', control.container );
	...
	/**
	 * Customizer Control: zenvy_select
	 */
	api.controlConstructor.zenvy_select = api.Control.extend( {
		...
		setValues() {
			...
			// Assign constant variables
			const value = ( control.setting._value !== '' ) ? control.setting._value : control.params.value;
	...
	/**
	 * Customizer Control: zenvy_icon_select
	 */
	api.controlConstructor.zenvy_icon_select = api.Control.extend( {
		...
		ready() {
			const control = this,
				...
				select2Options = {
					...
					allowClear: true,
					templateSelection: zenvy_format_Text,
					templateResult: zenvy_format_Text,
				};
	...
	api.controlConstructor.zenvy_multi_select = api.Control.extend( {
		...
		setValues() {
			...
			const multi_options = value.split( ',' );
